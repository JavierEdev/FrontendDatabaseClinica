// src/pages/admin/recetas/ListRecetas.tsx
...
import RecetaEditModal from "@/features/recetas/ui/RecetaEditModal";
        ...
        <div className={styles.header}>
          ...
          <div className={styles.headerActions}>
            <div className={styles.searchBox}>
              ...
              />
              {f.q && (
                <button className={styles.clearBtn} onClick={() => f.setQ("")}>
                  ×
                </button>
              )}
            </div>
            <Link to="/admin/recetas/crear" className={styles.primaryBtn}>
              + Crear recetas
            </Link>
          </div>
        </div>
        ...
        <div className={styles.footer}>
          <div className={styles.rows}>
            <label>
              ...
              <select
                value={f.pageSize}
                onChange={(e) => f.setPageSize(parseInt(e.target.value, 10))}
              >
                {[10, 20, 50].map((n) => (
          ...
          <div className={styles.pager}>
            <button
              onClick={() => f.setPage((p) => Math.max(1, p - 1))}
              disabled={loading || f.from <= 1}
            >
              ← Anterior
            </button>
            <span>Página {Math.ceil(f.from / f.pageSize) || 1}</span>
            <button
              onClick={() => f.setPage((p) => p + 1)}
              disabled={loading || f.to >= f.total}
            >
              Siguiente →
            </button>
          </div>
