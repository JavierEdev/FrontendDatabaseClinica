...
import { listarPacientes, subirDocumentoPaciente } from "@/features/pacientes/api/pacientes";
...
/** ===== Nuevo modal: subir imagen/documento ===== */
function ImageUploadModal({
  ...
  const [categoria, setCategoria] = useState<"dpi" | "resultado" | "seguro" | "imagen" | "otro">("imagen");
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      await subirDocumentoPaciente(paciente.idPaciente, file, categoria, notas.trim() || undefined);
      onUploaded?.();
      onClose();
      alert("Imagen/documento subido correctamente.");
    } catch (e: any) {
...
export default function PacientesListPage() {
  ...
  const [pacienteForImg, setPacienteForImg] = useState<PacienteItem | null>(null);
  ...
  const openImgFor = (p: PacienteItem) => { setPacienteForImg(p); setImgOpen(true); };
  const closeImg = () => { setImgOpen(false); setPacienteForImg(null); };
              ...
              {!loading && !err && items.map((p) => (
                <tr key={p.idPaciente}>
                  ...
                  <td className={`${styles.right} ${styles.actions}`}>
                    <button onClick={() => openImgFor(p)}>Agregar imagen</button>
                  </td>
                </tr>
              ))}
            </tbody>
